...
if(channel){channel.send(`🎉 ${message.author} leveled up to **Level ${userData.level}!**`);}}
else{message.channel.send(`🎉 ${message.author} leveled up to **Level ${userData.level}!**`);}}}
...
catch(e){clearInterval(game.interval);}},1500);}
...
if(!message.member.permissions.has(PermissionFlagsBits.Administrator))return message.reply('❌ Admins only.');
const embed=new EmbedBuilder().setTitle('Support Tickets').setDescription('Need assistance? Click the button below to open a ticket.\n\n**Rules:**\nBe respectful.\nProvide all relevant information.\nDo not ping staff repeatedly.\nStay on topic.').setColor(0xFF6600).setFooter({text:'Sour Bot'});
...
try{if(target.moderatable){await target.timeout(3600000,reason);}
...
const embed=new EmbedBuilder().setTitle('User Warned').setColor(0xFF6600).setThumbnail(target.user.displayAvatarURL()).addFields({name:'User',value:`${target.user.tag}`,inline:true},{name:'Moderator',value:`${message.author.tag}`,inline:true},{name:'Reason',value:reason},{name:'Active Warns',value:`${warnCount}/3`},{name:'Timeout',value:'1 hour'}).setTimestamp();
...
const embed=new EmbedBuilder().setTitle('User Banned').setColor(0xFF0000).setThumbnail(target.user.displayAvatarURL()).addFields({name:'User',value:`${target.user.tag}`,inline:true},{name:'Moderator',value:`${message.author.tag}`,inline:true},{name:'Reason',value:reason}).setTimestamp();
message.channel.send({embeds:[embed]});}
...
const embed=new EmbedBuilder().setTitle('User Kicked').setColor(0xFFA500).setThumbnail(target.user.displayAvatarURL()).addFields({name:'User',value:`${target.user.tag}`,inline:true},{name:'Moderator',value:`${message.author.tag}`,inline:true},{name:'Reason',value:reason}).setTimestamp();
message.channel.send({embeds:[embed]});}
...
const embed=new EmbedBuilder().setTitle('User Timed Out').setColor(0xFFFF00).setThumbnail(target.user.displayAvatarURL()).addFields({name:'User',value:`${target.user.tag}`,inline:true},{name:'Moderator',value:`${message.author.tag}`,inline:true},{name:'Duration',value:`${minutes} minutes`,inline:true},{name:'Reason',value:reason}).setTimestamp();
message.channel.send({embeds:[embed]});}
...
const playerPos=2;const spikePos=8;const score=0;
const display=buildGDDisplay(playerPos,spikePos,false,score);
const row=buildGDRow();
...
if(justPassed)g.jumped=false;
if(hit){
g.dead=true;clearInterval(g.interval);
const row=buildGDRow(true,false);
...
const input=new TextInputBuilder().setCustomId('ticket_reason').setLabel('What do you need help with?').setStyle(TextInputStyle.Paragraph).setPlaceholder('Describe your issue...').setMaxLength(1000).setRequired(true);
modal.addComponents(new ActionRowBuilder().addComponents(input));
await interaction.showModal(modal);}

else if(interaction.isModalSubmit()&&interaction.customId==='ticket_modal'){
const guildId=interaction.guild.id;const userId=interaction.user.id;
