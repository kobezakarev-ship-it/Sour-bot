...
// ── AI CHAT STATE ─────────────────────────────────────────────────────────────
const openAIChats = new Map();       // userId → channelId
...
client.on('messageCreate', async (message) => {
  ...
  // ── AI Chat reply (no prefix needed inside an AI channel) ──────────────────
  if (aiChatHistory.has(message.channel.id)) {
    const history = aiChatHistory.get(message.channel.id);
    ...
    try {
      ...
      const response = await groq.chat.completions.create({
        ...
        max_tokens: 1024,
        messages: [
          { role: 'system', content: 'You are a helpful AI assistant inside a Discord server. Be friendly, concise, and helpful.' },
          ...history,
        ],
      });
      ...
      // Split long replies to fit Discord's 2000 char limit
      ...
      await message.reply('❌ AI error — please try again in a moment.');
    }
  ...
  if (command === 'ai') {
    ...
    const isAdmin = message.member.roles.cache.has('1488466755440611449');
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton() && interaction.customId === 'create_ai_chat') {
    ...
    if (openAIChats.has(user.id)) {
      const existing = guild.channels.cache.get(openAIChats.get(user.id));
      if (existing) {
        return interaction.editReply(`❌ You already have an open AI chat: ${existing}`);
      } else {
        openAIChats.delete(user.id); // stale, clean up
      }
    }
    ...
    // Try to put it under an "ai-chats" category if it exists
    const category = guild.channels.cache.find(
      c => c.type === ChannelType.GuildCategory &&
           (c.name.toLowerCase().includes('ai') || c.name.toLowerCase().includes('chat'))
    );
    ...
    openAIChats.set(user.id, channel.id);
    aiChatHistory.set(channel.id, []); // start empty conversation history
    ...
    const welcomeEmbed = new EmbedBuilder()
      ...
      .setFooter({ text: `Powered by Claude AI • made by Purpleskeleton__ | Today at ${new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}` });
  ...
  // ── Close AI Chat button ───────────────────────────────────────────────────
    ...
    openAIChats.delete(userId);
    aiChatHistory.delete(interaction.channel.id);
